...
const FINALIZED_ROOT_INDEX := new GeneralizedIndex(105);
const NEXT_SYNC_COMMITTEE_INDEX := new GeneralizedIndex(55);
...
const TIMELY_HEAD_FLAG_INDEX := 0;
const TIMELY_SOURCE_FLAG_INDEX := 1;
const TIMELY_TARGET_FLAG_INDEX := 2;
const TIMELY_HEAD_WEIGHT := new uint64(12);
const TIMELY_SOURCE_WEIGHT := new uint64(12);
const TIMELY_TARGET_WEIGHT := new uint64(24);
const SYNC_REWARD_WEIGHT := new uint64(8);
const WEIGHT_DENOMINATOR := new uint64(64);
const G2_POINT_AT_INFINITY := new BLSSignature(pybytes.create("\xc0") + (pybytes.create("\x00") * 95));
const INACTIVITY_PENALTY_QUOTIENT_ALTAIR := new uint64(3 * 2.pow(24));
const MIN_SLASHING_PENALTY_QUOTIENT_ALTAIR := new uint64(2.pow(6));
const PROPORTIONAL_SLASHING_MULTIPLIER_ALTAIR := new uint64(2);
const SYNC_COMMITTEE_SIZE := new uint64(2.pow(10));
const SYNC_PUBKEYS_PER_AGGREGATE := new uint64(2.pow(6));
const INACTIVITY_SCORE_BIAS := new uint64(4);
const EPOCHS_PER_SYNC_COMMITTEE_PERIOD := new Epoch(2.pow(8));
const DOMAIN_SYNC_COMMITTEE := new DomainType("0x07000000");
const DOMAIN_SYNC_COMMITTEE_SELECTION_PROOF := new DomainType("0x08000000");
const DOMAIN_CONTRIBUTION_AND_PROOF := new DomainType("0x09000000");
const ALTAIR_FORK_VERSION := new Version("0x01000000");
const ALTAIR_FORK_SLOT := new Slot(0);
const TARGET_AGGREGATORS_PER_SYNC_SUBCOMMITTEE := 2.pow(2);
const SYNC_COMMITTEE_SUBNET_COUNT := 8;
const MIN_SYNC_COMMITTEE_PARTICIPANTS := 1;
const MAX_VALID_LIGHT_CLIENT_UPDATES := new uint64(2.pow(64) - 1);
const LIGHT_CLIENT_UPDATE_TIMEOUT := new Slot(2.pow(13));
...
datatype SyncCommittee = SyncCommittee(
  ...
  pubkey_aggregates: Vector<BLSPubkey>
)
...
datatype SyncCommitteeSignature = SyncCommitteeSignature(
  slot: Slot,
  beacon_block_root: Root,
  validator_index: ValidatorIndex,
  signature: BLSSignature
)
datatype SyncCommitteeContribution = SyncCommitteeContribution(
  slot: Slot,
  beacon_block_root: Root,
...
datatype ContributionAndProof = ContributionAndProof(
  aggregator_index: ValidatorIndex,
  contribution: SyncCommitteeContribution,
  selection_proof: BLSSignature
)
datatype SignedContributionAndProof = SignedContributionAndProof(
  message: ContributionAndProof,
  signature: BLSSignature
)
datatype SyncCommitteeSigningData = SyncCommitteeSigningData(
  slot: Slot,
  subcommittee_index: uint64
)
datatype LightClientSnapshot = LightClientSnapshot(
  header: BeaconBlockHeader,
  current_sync_committee: SyncCommittee,
  next_sync_committee: SyncCommittee
)
datatype LightClientUpdate = LightClientUpdate(
  header: BeaconBlockHeader,
  next_sync_committee: SyncCommittee,
  next_sync_committee_branch: Vector<Bytes32>,
  finality_header: BeaconBlockHeader,
  finality_branch: Vector<Bytes32>,
  sync_committee_bits: Bitvector,
  sync_committee_signature: BLSSignature,
  fork_version: Version
)
datatype LightClientStore = LightClientStore(
  snapshot: LightClientSnapshot,
  valid_updates: List<LightClientUpdate>
)
...
function method slash_validator(state_0: BeaconState,slashed_index_0: ValidatorIndex,whistleblower_index_0: ValidatorIndex): Unit {
  ...
  decrease_balance(state_0, slashed_index_0, validator_0.effective_balance / MIN_SLASHING_PENALTY_QUOTIENT_ALTAIR);
...
function method process_epoch(state_0: BeaconState): Unit {
  ...
  process_inactivity_updates(state_0);
...
function method process_justification_and_finalization(state_0: BeaconState): Unit {
  ...
  var previous_indices_0 := get_unslashed_participating_indices(state_0, TIMELY_TARGET_FLAG_INDEX, get_previous_epoch(state_0));
  var current_indices_0 := get_unslashed_participating_indices(state_0, TIMELY_TARGET_FLAG_INDEX, get_current_epoch(state_0));
  var total_active_balance_0 := get_total_active_balance(state_0);
  var previous_target_balance_0 := get_total_balance(state_0, previous_indices_0);
  var current_target_balance_0 := get_total_balance(state_0, current_indices_0);
...
function method get_base_reward(state_0: BeaconState,index_0: ValidatorIndex): Gwei {
  var increments_0 := state_0.validators[index_0].effective_balance / EFFECTIVE_BALANCE_INCREMENT;
  return new Gwei(increments_0 * get_base_reward_per_increment(state_0));
}
...
while i24 < |coll25|
  decreases |coll25| - i
{
var tmp_23 := coll25[i24];
      var __0 := tmp_23.first;
      var weight_0 := tmp_23.second;
        penalties_0 := penalties_0[index_0 := penalties_0[index_0] + new Gwei(get_base_reward(state_0, index_0) * weight_0 / WEIGHT_DENOMINATOR)];
      }
      if index_0 !in matching_target_indices_0 {
        var penalty_numerator_0 := state_0.validators[index_0].effective_balance * state_0.inactivity_scores[index_0];
        var penalty_denominator_0 := INACTIVITY_SCORE_BIAS * INACTIVITY_PENALTY_QUOTIENT_ALTAIR;
        penalties_0 := penalties_0[index_0 := penalties_0[index_0] + new Gwei(penalty_numerator_0 / penalty_denominator_0)];
      }
    }
  }
  return Pair(rewards_0, penalties_0);
...
function method process_rewards_and_penalties(state_0: BeaconState): Unit {
  ...
  var flag_indices_and_numerators_0 := get_flag_indices_and_weights();
  var flag_deltas_0 := flag_indices_and_numerators_0.map((tmp_26) => var index := tmp_26.first; var numerator := tmp_26.second; get_flag_index_deltas(state_0, index, numerator));
  var deltas_0 := flag_deltas_0 + PyList(get_inactivity_penalty_deltas(state_0));
  var i28 := 0;
var coll29 := i28;
while i28 < |coll29|
  decreases |coll29| - i
{
var tmp_27 := coll29[i28];
  var rewards_0 := tmp_27.first;
  var penalties_0 := tmp_27.second;
    var i30 := 0;
var coll31 := i30;
while i30 < |coll31|
  decreases |coll31| - i
{
var index_0 := coll31[i30];
...
while i33 < |coll34|
  decreases |coll34| - i
{
var tmp_32 := coll34[i33];
  var index_0 := tmp_32.first;
  var validator_0 := tmp_32.second;
  ...
  var activation_queue_0 := sorted(enumerate(state_0.validators).filter((tmp_35) => var index := tmp_35.first; var validator := tmp_35.second; is_eligible_for_activation(state_0, validator)).map((tmp_36) => var index := tmp_36.first; var validator := tmp_36.second; index), (index) => Pair(state_0.validators[index].activation_eligibility_epoch, index));
  var i37 := 0;
var coll38 := i37;
while i37 < |coll38|
  decreases |coll38| - i
{
var index_1 := coll38[i37];
...
function method process_slashings(state_0: BeaconState): Unit {
  var epoch_0 := get_current_epoch(state_0);
  var total_balance_0 := get_total_active_balance(state_0);
  var adjusted_total_slashing_balance_0 := min(sum(state_0.slashings) * PROPORTIONAL_SLASHING_MULTIPLIER_ALTAIR, total_balance_0);
  var i40 := 0;
var coll41 := i40;
while i40 < |coll41|
  decreases |coll41| - i
{
var tmp_39 := coll41[i40];
  var index_0 := tmp_39.first;
  var validator_0 := tmp_39.second;
...
while i43 < |coll44|
  decreases |coll44| - i
{
var tmp_42 := coll44[i43];
  var index_0 := tmp_42.first;
  var validator_0 := tmp_42.second;
...
function method process_block(state_0: BeaconState,block_0: BeaconBlock): Unit {
  ...
  process_sync_committee(state_0, block_0.body.sync_aggregate);
}
...
while i55 < |coll56|
  decreases |coll56| - i
{
var signed_header_0 := coll56[i55];
...
function method process_attestation(state_0: BeaconState,attestation_0: Attestation): Unit {
  ...
  if data_0.target.epoch == get_current_epoch(state_0) {
    var epoch_participation_0 := state_0.current_epoch_participation;
    var justified_checkpoint_0 := state_0.current_justified_checkpoint;
    var epoch_participation_2 := epoch_participation_0;
    justified_checkpoint_2 := justified_checkpoint_0;
  } else {
    var epoch_participation_1 := state_0.previous_epoch_participation;
    var justified_checkpoint_1 := state_0.previous_justified_checkpoint;
    var epoch_participation_2 := epoch_participation_1;
    justified_checkpoint_2 := justified_checkpoint_1;
  }
  var is_matching_head_0 := data_0.beacon_block_root == get_block_root_at_slot(state_0, data_0.slot);
  var is_matching_source_0 := data_0.source == justified_checkpoint_2;
  var is_matching_target_0 := data_0.target.root == get_block_root(state_0, data_0.target.epoch);
  assert(is_matching_source_0)
  assert(is_valid_indexed_attestation(state_0, get_indexed_attestation(state_0, attestation_0)))
  var participation_flag_indices_0 := new PyList<int>();
  if is_matching_head_0 && is_matching_target_0 && (state_0.slot <= (data_0.slot + MIN_ATTESTATION_INCLUSION_DELAY)) {
    participation_flag_indices_0.append(TIMELY_HEAD_FLAG_INDEX);
  }
  if is_matching_source_0 && (state_0.slot <= (data_0.slot + integer_squareroot(SLOTS_PER_EPOCH))) {
    participation_flag_indices_0.append(TIMELY_SOURCE_FLAG_INDEX);
  }
  if is_matching_target_0 && (state_0.slot <= (data_0.slot + SLOTS_PER_EPOCH)) {
    participation_flag_indices_0.append(TIMELY_TARGET_FLAG_INDEX);
  }
  var proposer_reward_numerator_0 := 0;
...
while i62 < |coll63|
  decreases |coll63| - i
{
var tmp_61 := coll63[i62];
    var flag_index_0 := tmp_61.first;
    var weight_0 := tmp_61.second;
      if (flag_index_0 in participation_flag_indices_0) && !has_flag(epoch_participation_2[index_0], flag_index_0) {
        epoch_participation_2 := epoch_participation_2[index_0 := add_flag(epoch_participation_2[index_0], flag_index_0)];
        var proposer_reward_numerator_1 := proposer_reward_numerator_3 + (get_base_reward(state_0, index_0) * weight_0);
  ...
  var proposer_reward_0 := new Gwei(proposer_reward_numerator_2 / (WEIGHT_DENOMINATOR * PROPOSER_REWARD_QUOTIENT));
...
function method process_deposit(state_0: BeaconState,deposit_0: Deposit): Unit {
  ...
  var amount_0 := deposit_0.data.amount;
  var validator_pubkeys_0 := state_0.validators.map((validator) => validator.pubkey);
  if pubkey_0 !in validator_pubkeys_0 {
    ...
    if bls.Verify(pubkey_0, signing_root_0, deposit_0.data.signature) {
      ...
      state_0.previous_epoch_participation.append(new ParticipationFlags(0));
      state_0.current_epoch_participation.append(new ParticipationFlags(0));
      state_0.inactivity_scores.append(new uint64(0));
    }
  } else {
...
while i66 < |coll67|
  decreases |coll67| - i
{
var slot_0 := coll67[i66];
    var i68 := 0;
var coll69 := i68;
while i68 < |coll69|
  decreases |coll69| - i
{
var index_0 := coll69[i68];
...
/*
    ...
    */
function method eth2_fast_aggregate_verify(pubkeys_0: Sequence<BLSPubkey>,message_0: Bytes32,signature_0: BLSSignature): bool {
  if (len(pubkeys_0) == 0) && (signature_0 == G2_POINT_AT_INFINITY) {
    return true;
  }
  return bls.FastAggregateVerify(pubkeys_0, message_0, signature_0);
}

function method get_flag_indices_and_weights(): Sequence<Tuple<int,uint64>> {
  return Triple(Pair(TIMELY_HEAD_FLAG_INDEX, TIMELY_HEAD_WEIGHT), Pair(TIMELY_SOURCE_FLAG_INDEX, TIMELY_SOURCE_WEIGHT), Pair(TIMELY_TARGET_FLAG_INDEX, TIMELY_TARGET_WEIGHT));
}

function method add_flag(flags_0: ParticipationFlags,flag_index_0: int): ParticipationFlags {
  var flag_0 := new ParticipationFlags(2.pow(flag_index_0));
  return flags_0 or flag_0;
}

function method has_flag(flags_0: ParticipationFlags,flag_index_0: int): bool {
  var flag_0 := new ParticipationFlags(2.pow(flag_index_0));
  return (flags_0 and flag_0) == flag_0;
}

/*
    Return the sequence of sync committee indices (which may include duplicate indices) for a given state and epoch.
    */
function method get_sync_committee_indices(state_0: BeaconState,epoch_0: Epoch): Sequence<ValidatorIndex> {
  var MAX_RANDOM_BYTE_0 := 2.pow(8) - 1;
  var base_epoch_0 := new Epoch((max(epoch_0 / EPOCHS_PER_SYNC_COMMITTEE_PERIOD, 1) - 1) * EPOCHS_PER_SYNC_COMMITTEE_PERIOD);
  ...
  var i_0 := 0;
  List<ValidatorIndex> sync_committee_indices_0 := PyList();
  var i_2 := i_0;
  while len(sync_committee_indices_0) < SYNC_COMMITTEE_SIZE {
    var shuffled_index_0 := compute_shuffled_index(new uint64(i_2 % active_validator_count_0), active_validator_count_0, seed_0);
    var candidate_index_0 := active_validator_indices_0[shuffled_index_0];
    var random_byte_0 := hash(seed_0 + uint_to_bytes(new uint64(i_2 / 32)))[i_2 % 32];
    var effective_balance_0 := state_0.validators[candidate_index_0].effective_balance;
    if (effective_balance_0 * MAX_RANDOM_BYTE_0) >= (MAX_EFFECTIVE_BALANCE * random_byte_0) {
...
/*
    ...
    */
function method get_sync_committee(state_0: BeaconState,epoch_0: Epoch): SyncCommittee {
  var indices_0 := get_sync_committee_indices(state_0, epoch_0);
  var pubkeys_0 := indices_0.map((index) => state_0.validators[index].pubkey);
  var partition_0 := range(0, len(pubkeys_0), SYNC_PUBKEYS_PER_AGGREGATE).map((i) => pubkeys_0[i..i + SYNC_PUBKEYS_PER_AGGREGATE]);
  var pubkey_aggregates_0 := partition_0.map((preaggregate) => bls.AggregatePKs(preaggregate));
  return new SyncCommittee(pubkeys_0, pubkey_aggregates_0);
}

function method get_base_reward_per_increment(state_0: BeaconState): Gwei {
  return new Gwei(EFFECTIVE_BALANCE_INCREMENT * BASE_REWARD_FACTOR / integer_squareroot(get_total_active_balance(state_0)));
}

/*
    Return the active and unslashed validator indices for the given epoch and flag index.
    */
function method get_unslashed_participating_indices(state_0: BeaconState,flag_index_0: int,epoch_0: Epoch): Set<ValidatorIndex> {
  assert(epoch_0 in Pair(get_previous_epoch(state_0), get_current_epoch(state_0)))
  List<ParticipationFlags> epoch_participation_2;
  if epoch_0 == get_current_epoch(state_0) {
    var epoch_participation_0 := state_0.current_epoch_participation;
  ...
  var participating_indices_0 := active_validator_indices_0.filter((i) => has_flag(epoch_participation_2[i], flag_index_0)).map((i) => i);
  return set(filter((index) => !state_0.validators[index].slashed, participating_indices_0));
}

/*
    Return the deltas for a given flag index by scanning through the participation flags.
    */
function method get_flag_index_deltas(state_0: BeaconState,flag_index_0: int,weight_0: uint64): Tuple<Sequence<Gwei>,Sequence<Gwei>> {
  var rewards_0 := PyList(new Gwei(0)) * len(state_0.validators);
  var penalties_0 := PyList(new Gwei(0)) * len(state_0.validators);
  ...
  var increment_0 := EFFECTIVE_BALANCE_INCREMENT;
  var unslashed_participating_increments_0 := get_total_balance(state_0, unslashed_participating_indices_0) / increment_0;
  var active_increments_0 := get_total_active_balance(state_0) / increment_0;
  var i70 := 0;
var coll71 := i70;
while i70 < |coll71|
  decreases |coll71| - i
{
var index_0 := coll71[i70];
    var base_reward_0 := get_base_reward(state_0, index_0);
    if index_0 in unslashed_participating_indices_0 {
      if is_in_inactivity_leak(state_0) {
        rewards_0 := rewards_0[index_0 := rewards_0[index_0] + new Gwei(base_reward_0 * weight_0 / WEIGHT_DENOMINATOR)];
      } else {
        var reward_numerator_0 := base_reward_0 * weight_0 * unslashed_participating_increments_0;
        rewards_0 := rewards_0[index_0 := rewards_0[index_0] + new Gwei(reward_numerator_0 / (active_increments_0 * WEIGHT_DENOMINATOR))];
      }
    } else {
      penalties_0 := penalties_0[index_0 := penalties_0[index_0] + new Gwei(base_reward_0 * weight_0 / WEIGHT_DENOMINATOR)];
    }
  }
  return Pair(rewards_0, penalties_0);
}

function method process_sync_committee(state_0: BeaconState,aggregate_0: SyncAggregate): Unit {
  var previous_slot_0 := new Slot(max(new int(state_0.slot), 1) - 1);
  var committee_indices_0 := get_sync_committee_indices(state_0, get_current_epoch(state_0));
  var included_indices_0 := zip(committee_indices_0, aggregate_0.sync_committee_bits).filter((tmp_72) => var index := tmp_72.first; var bit := tmp_72.second; bit).map((tmp_73) => var index := tmp_73.first; var bit := tmp_73.second; index);
  var committee_pubkeys_0 := state_0.current_sync_committee.pubkeys;
  var included_pubkeys_0 := zip(committee_pubkeys_0, aggregate_0.sync_committee_bits).filter((tmp_74) => var pubkey := tmp_74.first; var bit := tmp_74.second; bit).map((tmp_75) => var pubkey := tmp_75.first; var bit := tmp_75.second; pubkey);
  ...
  assert(eth2_fast_aggregate_verify(included_pubkeys_0, signing_root_0, aggregate_0.sync_committee_signature))
  var total_active_increments_0 := get_total_active_balance(state_0) / EFFECTIVE_BALANCE_INCREMENT;
  var total_base_rewards_0 := new Gwei(get_base_reward_per_increment(state_0) * total_active_increments_0);
  var max_epoch_rewards_0 := new Gwei(total_base_rewards_0 * SYNC_REWARD_WEIGHT / WEIGHT_DENOMINATOR);
  var max_slot_rewards_0 := new Gwei(max_epoch_rewards_0 * len(included_indices_0) / len(committee_indices_0) / SLOTS_PER_EPOCH);
  var committee_effective_balance_0 := sum(included_indices_0.map((index) => state_0.validators[index].effective_balance));
  var committee_effective_balance_1 := max(EFFECTIVE_BALANCE_INCREMENT, committee_effective_balance_0);
  var i76 := 0;
var coll77 := i76;
while i76 < |coll77|
  decreases |coll77| - i
{
var included_index_0 := coll77[i76];
    var effective_balance_0 := state_0.validators[included_index_0].effective_balance;
    var inclusion_reward_0 := new Gwei(max_slot_rewards_0 * effective_balance_0 / committee_effective_balance_1);
    var proposer_reward_0 := new Gwei(inclusion_reward_0 / PROPOSER_REWARD_QUOTIENT);
    increase_balance(state_0, get_beacon_proposer_index(state_0), proposer_reward_0);
    increase_balance(state_0, included_index_0, inclusion_reward_0 - proposer_reward_0);
  }
}

function method process_inactivity_updates(state_0: BeaconState): Unit {
  var i78 := 0;
var coll79 := i78;
while i78 < |coll79|
  decreases |coll79| - i
{
var index_0 := coll79[i78];
    if index_0 in get_unslashed_participating_indices(state_0, TIMELY_TARGET_FLAG_INDEX, get_previous_epoch(state_0)) {
      if state_0.inactivity_scores[index_0] > 0 {
        state_0 := state_0.(inactivity_scores := state_0.inactivity_scores[index_0 := state_0.inactivity_scores[index_0] - 1]]);
      }
    } else {
      if is_in_inactivity_leak(state_0) {
        state_0 := state_0.(inactivity_scores := state_0.inactivity_scores[index_0 := state_0.inactivity_scores[index_0] + INACTIVITY_SCORE_BIAS]]);
      }
    }
  }
}

function method process_participation_flag_updates(state_0: BeaconState): Unit {
  state_0 := state_0.(previous_epoch_participation := state_0.current_epoch_participation]);
  state_0 := state_0.(current_epoch_participation := new List<ParticipationFlags>(range(len(state_0.validators)).map((_0) => new ParticipationFlags(0)))]);
}

function method process_sync_committee_updates(state_0: BeaconState): Unit {
  var next_epoch_0 := get_current_epoch(state_0) + new Epoch(1);
  if (next_epoch_0 % EPOCHS_PER_SYNC_COMMITTEE_PERIOD) == 0 {
    state_0 := state_0.(current_sync_committee := state_0.next_sync_committee]);
    state_0 := state_0.(next_sync_committee := get_sync_committee(state_0, next_epoch_0 + EPOCHS_PER_SYNC_COMMITTEE_PERIOD)]);
  }
}

function method upgrade_to_altair(pre_0: BeaconState): BeaconState {
  var epoch_0 := get_current_epoch(pre_0);
  var post_0 := new BeaconState(pre_0.genesis_time, pre_0.genesis_validators_root, pre_0.slot, new Fork(pre_0.fork.current_version, ALTAIR_FORK_VERSION, epoch_0), pre_0.latest_block_header, pre_0.block_roots, pre_0.state_roots, pre_0.historical_roots, pre_0.eth1_data, pre_0.eth1_data_votes, pre_0.eth1_deposit_index, pre_0.validators, pre_0.balances, pre_0.randao_mixes, pre_0.slashings, range(len(pre_0.validators)).map((_0) => new ParticipationFlags(0)), range(len(pre_0.validators)).map((_0) => new ParticipationFlags(0)), pre_0.justification_bits, pre_0.previous_justified_checkpoint, pre_0.current_justified_checkpoint, pre_0.finalized_checkpoint, range(len(pre_0.validators)).map((_0) => new uint64(0)), BeaconState.current_sync_committee_default, BeaconState.next_sync_committee_default);
  post_0 := post_0.(current_sync_committee := get_sync_committee(post_0, get_current_epoch(post_0))]);
  post_0 := post_0.(next_sync_committee := get_sync_committee(post_0, get_current_epoch(post_0) + EPOCHS_PER_SYNC_COMMITTEE_PERIOD)]);
  return post_0;
}

function method compute_sync_committee_period(epoch_0: Epoch): uint64 {
  return epoch_0 / EPOCHS_PER_SYNC_COMMITTEE_PERIOD;
}

function method is_assigned_to_sync_committee(state_0: BeaconState,epoch_0: Epoch,validator_index_0: ValidatorIndex): bool {
  var sync_committee_period_0 := compute_sync_committee_period(epoch_0);
  var current_epoch_0 := get_current_epoch(state_0);
  var current_sync_committee_period_0 := compute_sync_committee_period(current_epoch_0);
  var next_sync_committee_period_0 := current_sync_committee_period_0 + 1;
  assert(sync_committee_period_0 in Pair(current_sync_committee_period_0, next_sync_committee_period_0))
  var pubkey_0 := state_0.validators[validator_index_0].pubkey;
  if sync_committee_period_0 == current_sync_committee_period_0 {
    return pubkey_0 in state_0.current_sync_committee.pubkeys;
  } else {
    return pubkey_0 in state_0.next_sync_committee.pubkeys;
  }
}

function method process_sync_committee_contributions(state_0: BeaconState,block_0: BeaconBlock,contributions_0: Set<SyncCommitteeContribution>): Unit {
  var sync_aggregate_0 := new SyncAggregate(SyncAggregate.sync_committee_bits_default, SyncAggregate.sync_committee_signature_default);
  var signatures_0 := new PyList<BLSSignature>();
  var i80 := 0;
var coll81 := i80;
while i80 < |coll81|
  decreases |coll81| - i
{
var contribution_0 := coll81[i80];
    var subcommittee_index_0 := contribution_0.subcommittee_index;
    var i83 := 0;
var coll84 := i83;
while i83 < |coll84|
  decreases |coll84| - i
{
var tmp_82 := coll84[i83];
    var index_0 := tmp_82.first;
    var participated_0 := tmp_82.second;
      if pybool(participated_0) {
        var participant_index_0 := (SYNC_COMMITTEE_SIZE / SYNC_COMMITTEE_SUBNET_COUNT * subcommittee_index_0) + index_0;
        sync_aggregate_0 := sync_aggregate_0.(sync_committee_bits := sync_aggregate_0.sync_committee_bits[participant_index_0 := new boolean(true)]]);
      }
    }
    signatures_0.append(contribution_0.signature);
  }
  sync_aggregate_0 := sync_aggregate_0.(sync_committee_signature := bls.Aggregate(signatures_0)]);
  block_0 := block_0.(body := block_0.body.(sync_aggregate := sync_aggregate_0])]);
}

function method get_sync_committee_signature(state_0: BeaconState,validator_index_0: ValidatorIndex,privkey_0: int): SyncCommitteeSignature {
  var epoch_0 := get_current_epoch(state_0);
  var domain_0 := get_domain(state_0, DOMAIN_SYNC_COMMITTEE, epoch_0);
  var signing_root_0 := compute_signing_root(get_block_root_at_slot(state_0, state_0.slot), domain_0);
  var signature_0 := bls.Sign(privkey_0, signing_root_0);
  return new SyncCommitteeSignature(state_0.slot, SyncCommitteeSignature.beacon_block_root_default, validator_index_0, signature_0);
}

function method compute_subnets_for_sync_committee(state_0: BeaconState,validator_index_0: ValidatorIndex): Sequence<uint64> {
  var target_pubkey_0 := state_0.validators[validator_index_0].pubkey;
  var sync_committee_indices_0 := enumerate(state_0.current_sync_committee.pubkeys).filter((tmp_85) => var index := tmp_85.first; var pubkey := tmp_85.second; pubkey == target_pubkey_0).map((tmp_86) => var index := tmp_86.first; var pubkey := tmp_86.second; index);
  return sync_committee_indices_0.map((index) => new uint64(index / (SYNC_COMMITTEE_SIZE / SYNC_COMMITTEE_SUBNET_COUNT)));
}

function method get_sync_committee_slot_signature(state_0: BeaconState,slot_0: Slot,subcommittee_index_0: uint64,privkey_0: int): BLSSignature {
  var domain_0 := get_domain(state_0, DOMAIN_SYNC_COMMITTEE_SELECTION_PROOF, compute_epoch_at_slot(slot_0));
  var signing_data_0 := new SyncCommitteeSigningData(slot_0, subcommittee_index_0);
  var signing_root_0 := compute_signing_root(signing_data_0, domain_0);
  return bls.Sign(privkey_0, signing_root_0);
}

function method is_sync_committee_aggregator(state_0: BeaconState,signature_0: BLSSignature): bool {
  var modulo_0 := max(1, SYNC_COMMITTEE_SIZE / SYNC_COMMITTEE_SUBNET_COUNT / TARGET_AGGREGATORS_PER_SYNC_SUBCOMMITTEE);
  return (bytes_to_uint64(hash(signature_0)[0..8]) % modulo_0) == 0;
}

function method get_contribution_and_proof(state_0: BeaconState,aggregator_index_0: ValidatorIndex,contribution_0: SyncCommitteeContribution,privkey_0: int): ContributionAndProof {
  var selection_proof_0 := get_sync_committee_slot_signature(state_0, contribution_0.slot, contribution_0.subcommittee_index, privkey_0);
  return new ContributionAndProof(aggregator_index_0, contribution_0, selection_proof_0);
}

function method get_contribution_and_proof_signature(state_0: BeaconState,contribution_and_proof_0: ContributionAndProof,privkey_0: int): BLSSignature {
  var contribution_0 := contribution_and_proof_0.contribution;
  var domain_0 := get_domain(state_0, DOMAIN_CONTRIBUTION_AND_PROOF, compute_epoch_at_slot(contribution_0.slot));
  var signing_root_0 := compute_signing_root(contribution_and_proof_0, domain_0);
  return bls.Sign(privkey_0, signing_root_0);
}

function method get_subtree_index(generalized_index_0: GeneralizedIndex): uint64 {
  return new uint64(generalized_index_0 % 2.pow(floorlog2(generalized_index_0)));
}

function method validate_light_client_update(snapshot_0: LightClientSnapshot,update_0: LightClientUpdate,genesis_validators_root_0: Root): Unit {
  assert(update_0.header.slot > snapshot_0.header.slot)
  var snapshot_period_0 := compute_epoch_at_slot(snapshot_0.header.slot) / EPOCHS_PER_SYNC_COMMITTEE_PERIOD;
  var update_period_0 := compute_epoch_at_slot(update_0.header.slot) / EPOCHS_PER_SYNC_COMMITTEE_PERIOD;
  assert(update_period_0 in Pair(snapshot_period_0, snapshot_period_0 + 1))
  if update_0.finality_header == new BeaconBlockHeader(BeaconBlockHeader.slot_default, BeaconBlockHeader.proposer_index_default, BeaconBlockHeader.parent_root_default, BeaconBlockHeader.state_root_default, BeaconBlockHeader.body_root_default) {
    var signed_header_0 := update_0.header;
    assert(update_0.finality_branch == range(floorlog2(FINALIZED_ROOT_INDEX)).map((_0) => new Bytes32()))
    var signed_header_2 := signed_header_0;
  } else {
    var signed_header_1 := update_0.finality_header;
    assert(is_valid_merkle_branch(hash_tree_root(update_0.header), update_0.finality_branch, floorlog2(FINALIZED_ROOT_INDEX), get_subtree_index(FINALIZED_ROOT_INDEX), update_0.finality_header.state_root))
    var signed_header_2 := signed_header_1;
  }
  if update_period_0 == snapshot_period_0 {
    var sync_committee_0 := snapshot_0.current_sync_committee;
    assert(update_0.next_sync_committee_branch == range(floorlog2(NEXT_SYNC_COMMITTEE_INDEX)).map((_0) => new Bytes32()))
    var sync_committee_2 := sync_committee_0;
  } else {
    var sync_committee_1 := snapshot_0.next_sync_committee;
    assert(is_valid_merkle_branch(hash_tree_root(update_0.next_sync_committee), update_0.next_sync_committee_branch, floorlog2(NEXT_SYNC_COMMITTEE_INDEX), get_subtree_index(NEXT_SYNC_COMMITTEE_INDEX), update_0.header.state_root))
    var sync_committee_2 := sync_committee_1;
  }
  assert(sum(update_0.sync_committee_bits) >= MIN_SYNC_COMMITTEE_PARTICIPANTS)
  var participant_pubkeys_0 := zip(update_0.sync_committee_bits, sync_committee_2.pubkeys).filter((tmp_87) => var bit := tmp_87.first; var pubkey := tmp_87.second; bit).map((tmp_88) => var bit := tmp_88.first; var pubkey := tmp_88.second; pubkey);
  var domain_0 := compute_domain(DOMAIN_SYNC_COMMITTEE, update_0.fork_version, genesis_validators_root_0);
  var signing_root_0 := compute_signing_root(signed_header_2, domain_0);
  assert(bls.FastAggregateVerify(participant_pubkeys_0, signing_root_0, update_0.sync_committee_signature))
}

function method apply_light_client_update(snapshot_0: LightClientSnapshot,update_0: LightClientUpdate): Unit {
  var snapshot_period_0 := compute_epoch_at_slot(snapshot_0.header.slot) / EPOCHS_PER_SYNC_COMMITTEE_PERIOD;
  var update_period_0 := compute_epoch_at_slot(update_0.header.slot) / EPOCHS_PER_SYNC_COMMITTEE_PERIOD;
  if update_period_0 == (snapshot_period_0 + 1) {
    snapshot_0 := snapshot_0.(current_sync_committee := snapshot_0.next_sync_committee]);
    snapshot_0 := snapshot_0.(next_sync_committee := update_0.next_sync_committee]);
  }
  snapshot_0 := snapshot_0.(header := update_0.header]);
}

function method process_light_client_update(store_0: LightClientStore,update_0: LightClientUpdate,current_slot_0: Slot,genesis_validators_root_0: Root): Unit {
  validate_light_client_update(store_0.snapshot, update_0, genesis_validators_root_0);
  store_0.valid_updates.append(update_0);
  if ((sum(update_0.sync_committee_bits) * 3) > (len(update_0.sync_committee_bits) * 2)) && (update_0.finality_header != new BeaconBlockHeader(BeaconBlockHeader.slot_default, BeaconBlockHeader.proposer_index_default, BeaconBlockHeader.parent_root_default, BeaconBlockHeader.state_root_default, BeaconBlockHeader.body_root_default)) {
    apply_light_client_update(store_0.snapshot, update_0);
    store_0 := store_0.(valid_updates := new List<LightClientUpdate>()]);
  } else {
    if current_slot_0 > (store_0.snapshot.header.slot + LIGHT_CLIENT_UPDATE_TIMEOUT) {
      apply_light_client_update(store_0.snapshot, max(store_0.valid_updates, (update) => sum(update.sync_committee_bits)));
      store_0 := store_0.(valid_updates := new List<LightClientUpdate>()]);
    }
  }
}
