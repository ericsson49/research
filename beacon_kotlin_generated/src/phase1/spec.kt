...
fun get_committee_count_per_slot(state: BeaconState, epoch: Epoch): uint64 {
  return max(uint64(1uL), min(get_active_shard_count(state), uint64(len(get_active_validator_indices(state, epoch))) / SLOTS_PER_EPOCH / TARGET_COMMITTEE_SIZE))
}
...
fun initialize_beacon_state_from_eth1(eth1_block_hash: Bytes32, eth1_timestamp: uint64, deposits: Sequence<Deposit>): BeaconState {
  ...
  val state = BeaconState(genesis_time = eth1_timestamp + GENESIS_DELAY, fork = fork, eth1_data = Eth1Data(block_hash = eth1_block_hash, deposit_count = len(deposits)), latest_block_header = BeaconBlockHeader(body_root = hash_tree_root(BeaconBlockBody())), randao_mixes = PyList(eth1_block_hash) * EPOCHS_PER_HISTORICAL_VECTOR)
...
fun get_forkchoice_store(anchor_state: BeaconState, anchor_block: BeaconBlock): Store {
  ...
  return Store(time = anchor_state.genesis_time + (SECONDS_PER_SLOT * anchor_state.slot), genesis_time = anchor_state.genesis_time, justified_checkpoint = justified_checkpoint, finalized_checkpoint = finalized_checkpoint, best_justified_checkpoint = justified_checkpoint, blocks = PyDict(anchor_root to copy(anchor_block)), block_states = PyDict(anchor_root to anchor_state.copy()), checkpoint_states = PyDict(justified_checkpoint to anchor_state.copy()), shard_stores = range(get_active_shard_count(anchor_state)).map { shard -> Pair(Shard(shard), get_forkchoice_shard_store(anchor_state, Shard(shard))) }.toPyDict())
}
...
fun process_chunk_challenge(state: BeaconState, challenge: CustodyChunkChallenge): Unit {
  ...
  val max_attestation_challenge_epoch = Epoch(challenge.attestation.data.target.epoch + MAX_CHUNK_CHALLENGE_DELAY)
...
fun process_chunk_challenge_response(state: BeaconState, response: CustodyChunkResponse): Unit {
  ...
  assert(is_valid_merkle_branch(leaf = hash_tree_root(response.chunk), branch = response.branch, depth = CUSTODY_RESPONSE_DEPTH + 1uL, index = response.chunk_index, root = challenge.data_root))
...
fun process_early_derived_secret_reveal(state: BeaconState, reveal: EarlyDerivedSecretReveal): Unit {
  ...
  val derived_secret_location = uint64(reveal.epoch % EARLY_DERIVED_SECRET_PENALTY_MAX_FUTURE_EPOCHS)
...
fun compute_committee_source_epoch(epoch: Epoch, period: uint64): Epoch {
  var source_epoch = Epoch(epoch - (epoch % period))
...
fun get_shard_committee(beacon_state: BeaconState, epoch: Epoch, shard: Shard): Sequence<ValidatorIndex> {
  ...
  return compute_committee(indices = active_validator_indices, seed = seed, index = shard, count = get_active_shard_count(beacon_state))
}
...
fun get_light_client_committee(beacon_state: BeaconState, epoch: Epoch): Sequence<ValidatorIndex> {
  ...
  return compute_committee(indices = active_validator_indices, seed = seed, index = uint64(0uL), count = get_active_shard_count(beacon_state))[0uL until LIGHT_CLIENT_COMMITTEE_SIZE]
}
...
fun get_shard_proposer_index(beacon_state: BeaconState, slot: Slot, shard: Shard): ValidatorIndex {
  ...
  val r = bytes_to_uint64(seed[0uL until 8uL])
...
fun get_committee_count_delta(state: BeaconState, start_slot: Slot, stop_slot: Slot): uint64 {
  return uint64(sum(range(start_slot, stop_slot).map { slot -> get_committee_count_per_slot(state, compute_epoch_at_slot(Slot(slot))) }))
}
...
fun get_start_shard(state: BeaconState, slot: Slot): Shard {
  ...
  if (current_epoch_start_slot == slot) {
    ...
    if (slot > current_epoch_start_slot) {
      ...
      val max_committees_per_slot = active_shard_count
      val max_committees_in_span = max_committees_per_slot * (current_epoch_start_slot - slot)
      return Shard((state.current_epoch_start_shard + max_committees_in_span - shard_delta) % active_shard_count)
    }
  }
}
...
fun apply_shard_transition(state: BeaconState, shard: Shard, transition: ShardTransition): Unit {
  ...
  val shard_state = copy(transition.shard_states[len(transition.shard_states) - 1uL])
