...
fun canBeCoercedTo(a: RTType, b: RTType): Boolean = when {
  isSubType(b, NamedType("boolean")) && isSubType(a, TPyBool) -> true
  b == TPyBool && a.typeInfo.coercibleToBool -> true
  b is NamedType && (b.name == "List" || b.name == "Vector") && isSubType(a, TPyIterable(b.tParams[0])) -> true
  isSubType(b, NamedType("uint")) && isSubType(a, TPyInt) -> true
...
object TypeResolver {
  ...
  private val funcResolvers = mutableMapOf<String, (List<RTType>, List<Pair<String, RTType>>) -> RTType>()
  ...
  fun resolveNameTyp(name: String): Sort? {
    val res = if (name in metaclasses) {
      metaclasses[name]
  ...
  fun registerFunc(f: FunctionDef) {
    registerFunc(f.name, getFunArgs(f), parseType(f.returns!!))
  }
  ...
  fun resolveBinopType(x: RTType, op: EBinOp, y: RTType): RTType? {
    return if (isSubType(x, NamedType("uint")) && isSubType(y, TPyInt)) {
    ...
    } else if (x is NamedType && isGenType(x) && (x.name == "PyList" || x.name == "List")) {
  ...
  fun resolveCompOpType(l: RTType, op: ECmpOp, r: RTType): Boolean {
    val lti = l.typeInfo!!
    val rti = r.typeInfo!!
    return when (op) {
  ...
  fun resolveUnaryOp(e: UnaryOp, type: RTType) =
          if (isSubType(type, TPyInt) && (e.op == EUnaryOp.UAdd || e.op == EUnaryOp.USub)) {
            TPyInt
          } else if (canAssignOrCoerceTo(type, TPyBool) && e.op == EUnaryOp.Not) {
            TPyBool
          } else {
            TODO()
          }
  ...
  fun resolveReturnType(callable: Sort, argTypes: List<RTType>, kwdArgs: List<Pair<String, RTType>>): Pair<FunSignature,List<ArgRef>> {
    return when (callable) {
      is NamedFuncRef -> {
        ...
        if (name in funcSigs) {
          funcSigs[name]!!.resolveFP(argTypes, kwdArgs)
                  ?: fail("no function found for $name($argTypes,$kwdArgs)")
        } else {
...
fun getFunArgs(f: FunctionDef): List<FArg> {
  val args = f.args
  ...
  return args.args.zip(defaults).map { getFArg(it.first, it.second) }
}
...
class ExprTypes(val ctx: NameResolver<Sort>, val cache: IdentityHashMap<TExpr, Sort>) {
  ...
  fun getExprType(e: TExpr, ctx: NameResolver<Sort>): Sort {
    val res = when (e) {
      ...
      is UnaryOp -> {
        TypeResolver.resolveUnaryOp(e, getExprType(e.operand, ctx).asType())
      }
      is BoolOp -> {
        e.values.forEach {
          val type = getExprType(it, ctx).asType()
          if (!canAssignOrCoerceTo(type, TPyBool))
      ...
      is Compare -> {
        val leftType = getExprType(e.left, ctx).asType()
        val comparatorTypes = e.comparators.map { getExprType(it, ctx).asType() }
        val lefts = listOf(leftType).plus(comparatorTypes.subList(0, comparatorTypes.size - 1))
        val checks = lefts.zip(e.ops).zip(comparatorTypes).map {
          val (l, op) = it.first
          val r = it.second
          TypeResolver.resolveCompOpType(l, op, r)
        }
        if (!checks.all { it })
      ...
      is BinOp -> {
        TypeResolver.resolveBinopType(
                getExprType(e.left, ctx).asType(),
                e.op, getExprType(e.right, ctx).asType())
            ?: fail("unknown op $e")
      }
      is ListComp -> {
        if (e.generators.size != 1) {
          ...
          TPyList(getExprType(e.elt, newCtx).asType())
        }
      }
      is DictComp -> {
        if (e.generators.size != 1) {
          ...
          val vtPairs = matchVarsAndTypes(gen.target, getIterableElemType(getExprType(gen.iter, ctx).asType()))
          val newCtx = ctx.copy(vtPairs.map { it.first.id to it.second }.toMap())
          TPyDict(getExprType(e.key, newCtx).asType(), getExprType(e.value, newCtx).asType())
        }
      }
      is GeneratorExp -> {
        if (e.generators.size != 1) {
      ...
      is PyList -> {
        val types = e.elts.map { getExprType(it, ctx).asType() }.toSet()
        if (types.isEmpty())
          TPyList(NamedType("?"))
        else
          TPyList(getCommonSuperType(types))
      }
      is PyDict -> {
        val keyTypes = e.keys.map { getExprType(it, ctx).asType() }.toSet()
        val keyType = if (keyTypes.isEmpty()) NamedType("?")
        else getCommonSuperType(keyTypes)
        val valTypes = e.values.map { getExprType(it, ctx).asType() }.toSet()
        val valType = if (valTypes.isEmpty()) NamedType("?")
        else getCommonSuperType(valTypes)

        TPyDict(keyType, valType)
      }
