  ...
  override fun transform(e: TExpr, store: Boolean): TExpr {
    return when(e) {
      ...
      is Compare -> {
        ...
        transform(BoolOp(op = EBoolOp.And, values = exprs))
      }
...
class Desugar {
  ...
  fun mkAttribute(v: String, a: identifier, store: Boolean = false) = Attribute(mkName(v), a, mkCtx(store))
  fun mkSubscript(v: String, s: TSlice, store: Boolean = false) = Subscript(mkName(v), s, mkCtx(store))
...
fun main() {
  ...
  fDefs.forEach {
    pyPrintFunc(desugar(transformForOps(it)))
    println()
  }
}