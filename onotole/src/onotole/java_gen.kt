...
class JavaGen(val packageName: String, val rootPath: String, currPkg: String, importedPkgs: Set<String>): BaseGen(currPkg, importedPkgs) {
  ...
  override fun genVarAssignment(isVar: Boolean?, name: LVInfo, typ: String?, value: String?): String = genExprStmt(when(name) {
    is LVName -> {
      val typV = typ ?: "var"
  ...
  override fun genAugAssignment(lhs: LVInfo, op: EBinOp, rhs: RExpr) = genVarAssignment(
