...
fun newDefsAnalysis(s: Stmt, newDefsAnno: StmtAnnoMap<Set<String>>): Set<String> {
  if (s !in newDefsAnno) {
    newDefsAnno[s] = when (s) {
      is Return -> emptySet()
      is Assign -> if (s.targets.size == 1) getVarNamesInStoreCtx(s.targets[0]).toSet() else fail("unsupported")
      is AnnAssign -> getVarNamesInStoreCtx(s.target).toSet()
      is AugAssign -> getVarNamesInStoreCtx(s.target).toSet()
      is For -> if (s.orelse.isEmpty()) newDefsAnalysis(s.body, newDefsAnno) else fail("not yet implemented")
      is While -> if (s.orelse.isEmpty()) newDefsAnalysis(s.body, newDefsAnno) else fail("not yet implemented")
  ...
  fun varDefAnalysis(
    ...
    val res: List<Pair<String,VarDef>> = when (s) {
      ...
      is Assign -> {
        if (s.targets.size != 1)
          fail("unsupported")
        val names = getVarNamesInStoreCtx(s.targets[0])
