...
class Vars(val versions: MutableMap<String, Int> = mutableMapOf(),
  ...
  fun remove(v: String) {
    for (i in 0..versions[v]!!) {
      vars.remove(VarNameVersion(v, i))
    }
    versions.remove(v)
  }
}
...
private fun Vars.copyWithLiveVars(liveVars: Set<String>): Vars {
  val res = this.copy()
  val actualVars = res.versions.keys
  for (v in actualVars.minus(liveVars)) {
    res.remove(v)
  }
  return res
}
...
class VarSlotAnalysis(
  ...
  fun procStmt(s: Stmt, vars: Vars) {
    when(s) {
      is Assign -> {
        ...
        when(target) {
          is Name -> {
            ...
            val vs = vars[vName]
            if (vs?.refs == null) {
              vars[vName] = varInfo.newVarSlot(vName, vars.newVarName(vName), s)
            } else {
              vs.refs.add(s)
            }
          }
      ...
      is AugAssign -> {
        when (s.target) {
          is Name -> {
            ...
            val vs = vars[vName]
            if (vs == null) {
              fail("$vName is undefined")
            } else if (vs.refs == null) {
              vars[vName] = varInfo.newVarSlot(vName, vars.newVarName(vName), s)
            } else {
              vs.refs.add(s)
            }
      ...
      is If -> {
        val bodyVars = vars.copy() //WithLiveVars(liveVars[s]!!)
        ...
        val elseVars = vars.copy() //WithLiveVars(liveVars[s]!!)
...
fun varSlotAnalysis(f: FunctionDef): VarInfo {
  ...
  val args = f.args.args.map { it.arg }
  ...
  varInfo.phiWebMap.values.toSet().forEach {
    val ld = it.lastDef!!
    /*val s = (it.defStmt!! as StmtVarDef).s
...
fun main() {
  val defs = parseDefs("../eth2.0-specs/tests/fork_choice/defs_test.txt").map { it as FunctionDef }
  val funcs = defs.subList(0,1)

  funcs.forEach {
    println(it.name)
    val varInfo = varSlotAnalysis(it)
